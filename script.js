...
function createGlobe() {
  ...
  function seedNodes() {
    ...
    const clusters = [
      { lat: 45, lon: -100, spreadLat: 22, spreadLon: 34, count: 20, color: "cyan" },
      { lat: -15, lon: -60, spreadLat: 18, spreadLon: 18, count: 10, color: "cyan" },
      { lat: 50, lon: 10, spreadLat: 18, spreadLon: 26, count: 18, color: "gold" },
      { lat: 5, lon: 20, spreadLat: 22, spreadLon: 22, count: 12, color: "cyan" },
      { lat: 28, lon: 45, spreadLat: 16, spreadLon: 18, count: 10, color: "gold" },
      { lat: 35, lon: 105, spreadLat: 24, spreadLon: 34, count: 18, color: "cyan" },
  ...
  function draw() {
    ...
    projectedCenters.forEach((hub) => {
      ...
      ctx.arc(hub.x, hub.y, 4.5 + hub.scale * 1.3, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
    });
